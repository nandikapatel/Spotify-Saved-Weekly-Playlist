...
# set the name of the session to cookie
app.config['SEESION_COOKIE_NAME'] = 'Spotify Cookie'

# set a randon secret key to sign the cookie 
app.secret_key = 'YOUR_SECRET_KEY'

# set the key for the token info in the session dictionary
TOKEN_INFO = 'token_info'

#route to handle the logging in
@app.route('/')
def login():
...
def redirect_page():
    ...
    # exchange the authorization code for an access token and refresh the token 
...
def save_discover_weekly():
    ...
    # if the Saved Weekly playlist is not found, make a new playlist
    ...
    return ("Songs from the Discover Weekly playlist successfully added!")

# function to get the token into from the session
def get_token():
    token_info = session.get(TOKEN_INFO, None)
    if not token_info:
        # is the token is not found, redirect the user to the login route
        redirect(url_for('login', extra = False))
